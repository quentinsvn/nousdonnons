...
if (isset($_SESSION['id'])) {
    ...
    // Récupération nombre de membres
    $members = $database->query("SELECT COUNT(id) FROM users")->fetchColumn();

    // Récupération données des annonces
    $annonces = $database->query("SELECT COUNT(id) FROM annonces WHERE publish = 1")->fetchColumn();
} else {
    ...
    <div class="wrapper">
        ...
        <div id="content">
            ...
            <div class="title_content mb-3">
                <h2>Liste des annonces acceptées</h2>
                <span><i class="fas fa-home"></i> <a href="#">Tableau de bord > Annonces (Acceptées)</a></span>
            </div>
                  ...
                  <div class="col-12 col-sm-6 card_last_members_col mb-3">
                      <div class="card_last_members" style="border-color: #e74c3c;">
                          <div class="title_last_members ml-2 mt-2 mb-3">
                              <h5>Liste des annonces acceptées</h5>
                          </div>
                          <table class="table">
                              <tbody>
                                  <?php
                                      $query = $database->prepare("SELECT * FROM annonces WHERE publish = 1 LIMIT 10");
                                      $query->execute();
