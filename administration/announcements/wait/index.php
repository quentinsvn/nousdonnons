...
if (isset($_SESSION['id'])) {
    ...
    $annonces = $database->query("SELECT COUNT(id) FROM annonces WHERE publish = 0")->fetchColumn();
} else {
    ...
    <div class="wrapper">
                ...
                <ul class="list-unstyled components">
                    ...
                    <li>
                        ...
                        <ul class="collapse list-unstyled" id="pageSubmenu">
                            <li>
                                <a href="../index.php"><i class="fas fa-eye" aria-hidden="true"></i> Voir tout</a>
                            </li>
                            <li>
                                <a href="../wait"><i class="fas fa-pause" aria-hidden="true"></i> En attentes</a>
                            </li>
                            <li>
                                <a href="../valid"><i class="fas fa-check-circle" aria-hidden="true"></i> Acceptées</a>
                            </li>
                        </ul>
                    </li>
                </ul>
        ...
        <div id="content">
            ...
            <div class="title_content mb-3">
                <h2>Liste des annonces en attentes</h2>
                <span><i class="fas fa-home"></i> <a href="#">Tableau de bord > Annonces (En attente)</a></span>
            </div>
                  ...
                  <div class="col-12 col-sm-6 card_last_members_col mb-3">
                      <div class="card_last_members" style="border-color: #e74c3c;">
                          <div class="title_last_members ml-2 mt-2 mb-3">
                              <h5>Liste des annonces en attentes</h5>
                          </div>
                          <table class="table">
                              <tbody>
                                  <?php
                                      $query = $database->prepare("SELECT * FROM annonces WHERE publish = 0 LIMIT 10");
                                      $query->execute();
                                  ...
                                  <tr>
                                      ...
                                      <?php
                                          ...
                                          if (isset($_POST['valider'])) {
                                              $annoucement_id = $row['id'];
                                              $update_name = $database->prepare("UPDATE annonces SET publish = ? WHERE id = ?");
                                              $update_name->execute(array(1, $annoucement_id));
                                              $message = "<div style='text-align: center;' class='alert alert-success'>Annonce validée.</div>";
                                          }
                                         ...
                                         ?>
                                      <form method="POST">
                                        <td><button id="see" type="submit" name="valider" style="background-color: #2ecc71;"><i class="fas fa-check-circle" aria-hidden="true"></i> Valider</button></td>
                                      </form>
